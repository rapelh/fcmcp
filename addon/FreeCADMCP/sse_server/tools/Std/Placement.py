...
def do_it(args):
    ...
    if mode == "AAP":
        ...
        sse_request_queue.put(lambda: _aap_placement_gui(doc_name, obj_name, angle, axis, position, center))
        ...
        sse_request_queue.put(lambda: _pypr_placement_gui(doc_name, obj_name, position, yaw, pitch, roll, center))
        res = sse_response_queue.get()
        if res is True:
