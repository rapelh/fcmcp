...
tool_type = types.Tool(
                name="Draft-Arc-ByThreeVectors",
                description="Create a labeled arc object from 3 vectors in a named document",
                inputSchema={
                    "type": "object",
                    "required": ["DocName"],
                    "properties": {
...
def do_it(args):
    ...
    rpc_request_queue.put(lambda: _arc_by_three_vectors_gui(doc_name, label, probj))
    res, text = rpc_response_queue.get()
    return [types.TextContent(type="text", text=text)]

def _arc_by_three_vectors_gui(doc_name, label, probj):
    ...
    p3 = FreeCAD.Vector(probj.properties["X3"], probj.properties["Y3"], probj.properties["Z3"])
