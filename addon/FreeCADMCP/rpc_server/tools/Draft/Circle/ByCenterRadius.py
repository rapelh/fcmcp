...
tool_type = types.Tool(
                name="Draft-Circle-ByCenterRadius",
                description="Create a labeled circle object from center and radius in a named document",
                inputSchema={
                    "type": "object",
                    "required": ["DocName"],
                    "properties": {
...
def _arc_by_center_radius_angles_gui(doc_name, label, probj):
    ...
    if position:
        ...
        pos[1] = position["Y"]
        pos[2] = position["Z"]
    circle = Draft.make_circle(probj.properties["Radius"])
    rot = FreeCAD.Rotation(FreeCAD.Vector(0, 0, 1), 0)
    circle.Placement = FreeCAD.Placement(pos, rot, ctr)
    circle.Label = label
    doc.recompute()
    try:
